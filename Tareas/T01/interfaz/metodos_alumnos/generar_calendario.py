def generar_calendario(cursos_por_tomar):
    ...
    if not cursos_por_tomar:
        print("No tiene cursos inscritos aun.")
        return False
    ...
    for curso in cursos_por_tomar:
        ...
        if curso.evaluaciones:
            ...
            for evaluacion in curso.evaluaciones:
                texto_evaluaciones += "    - {0}: el dia {1} a las {2} horas".format(evaluacion.nombre, evaluacion.dia, evaluacion.hora)
                texto_evaluaciones += '\n'
            texto_evaluaciones += '\n'

        else:
            texto_evaluaciones += "   --- El curso no tiene evaluaciones registradas. ---\n\n"
    ...
    if hay_evaluaciones:
        ...
        texto_final = "\n--- Ninguno de sus cursos tiene evaluaciones registradas ---"
